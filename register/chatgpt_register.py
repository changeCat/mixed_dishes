...
from curlcffi import requests as curl_requests
...
def _load_config():
    ...
    config = {
        ...
        "continue_on_missing_duckmail_bearer": False,
        "telegram_bot_token": "",
        "telegram_chat_id": "",
    ...
    config["cpa_cleanup_enabled"] = os.environ.get("CPA_CLEANUP_ENABLED", config["cpa_cleanup_enabled"])
    config["continue_on_missing_duckmail_bearer"] = os.environ.get(
        "CONTINUE_ON_MISSING_DUCKMAIL_BEARER", config["continue_on_missing_duckmail_bearer"]
    )
    ...
    config["telegram_notify_enabled"] = os.environ.get(
        "TELEGRAM_NOTIFY_ENABLED", config["telegram_notify_enabled"]
    )
...
DEFAULT_MAX_WORKERS = _CONFIG.get("max_workers", 3)
DEFAULT_PROXY = (_CONFIG["proxy"] or "").strip() or None
...
CONTINUE_ON_MISSING_DUCKMAIL_BEARER = _as_bool(_CONFIG.get("continue_on_missing_duckmail_bearer", False))
TELEGRAM_BOT_TOKEN = (_CONFIG.get("telegram_bot_token", "") or "").strip()
TELEGRAM_CHAT_ID = str(_CONFIG.get("telegram_chat_id", "") or "").strip()
TELEGRAM_NOTIFY_ENABLED = _as_bool(_CONFIG.get("telegram_notify_enabled", False))
...
def _upload_all_tokens_to_cpa():
    ...
    print(f"{'='*60}")
    return {"found": len(json_files), "uploaded": uploaded, "failed": failed}
...
def _run_cpa_cleanup_before_register():
    ...
    try:
        ...
        result["remaining_total"] = max(0, int(result.get("scanned_total", 0)) - int(result.get("deleted_total", 0)))
        print(f"\n  [CPA清理] 清理完成: 扫描 {result['scanned_total']} 个, "
              f"命中 {result['matched_total']} 个, 删除 {result['deleted_total']} 个")
        print(f"  [CPA清理] 清理后剩余: {result['remaining_total']} 个")
        if result["failures"]:
...
def _send_telegram_summary(summary):
    if not TELEGRAM_NOTIFY_ENABLED:
        return False
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
        print("[TG] ⚠️ 未配置 telegram_bot_token 或 telegram_chat_id，跳过推送")
        return False

    cleanup = summary.get("cleanup", {}) or {}
    register = summary.get("register", {}) or {}
    upload = summary.get("upload", {}) or {}

    message = "\n".join([
        "【ChatGPT Register 每日报告】",
        f"CPA清理数量：{cleanup.get('deleted_total', 0)}",
        f"清理后剩余：{cleanup.get('remaining_total', 0)}",
        f"注册数量：{register.get('total_accounts', 0)}",
        f"注册成功：{register.get('success_count', 0)}",
        f"注册失败：{register.get('fail_count', 0)}",
        f"导入后有多少：{upload.get('uploaded', 0)}",
    ])
    ...
    try:
        ...
        if resp.status_code == 200:
            print("[TG] ✅ Telegram 推送成功")
            return True
        print(f"[TG] ❌ Telegram 推送失败: {resp.status_code} - {resp.text[:200]}")
    except Exception as e:
        print(f"[TG] ❌ Telegram 推送异常: {e}")
    return False
...
def run_batch(total_accounts: int = 3, output_file="registered_accounts.txt",
    ...
    if do_cleanup and UPLOAD_API_URL:
        cleanup_result = _run_cpa_cleanup_before_register() or cleanup_result
    ...
    upload_result = {"found": 0, "uploaded": 0, "failed": 0}

    # 注册流程结束后，自动上传到 CPA 并清理
    if success_count > 0:
        upload_result = _upload_all_tokens_to_cpa() or upload_result
...
def main():
    ...
    if not DUCKMAIL_BEARER:
        ...
        if not CONTINUE_ON_MISSING_DUCKMAIL_BEARER:
            print("   当前配置 continue_on_missing_duckmail_bearer=false，程序终止")
            return
        print("   当前配置 continue_on_missing_duckmail_bearer=true，继续尝试运行")
    ...
    if UPLOAD_API_URL:
        print(f"[Info] CPA 清理: {'开启' if cpa_cleanup else '关闭'}")
    else:
        print("[Info] 未配置 upload_api_url，跳过 CPA 清理")

    total_accounts = DEFAULT_TOTAL_ACCOUNTS if DEFAULT_TOTAL_ACCOUNTS > 0 else 3
    max_workers = DEFAULT_MAX_WORKERS if DEFAULT_MAX_WORKERS > 0 else 3
